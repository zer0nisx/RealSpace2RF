...
#include "RVisualMeshConstants.h"
#include <functional>
...
AniFrameInfo::AniFrameInfo()
{
	...
	m_fMaxBlendTime = AnimationConstants::DEFAULT_BLEND_TIME;
	m_fCurrentBlendTime = AnimationConstants::DEFAULT_CURRENT_BLEND_TIME;
	m_dwBackupBlendTime = 0;

	m_fSpeed = AnimationConstants::DEFAULT_ANIMATION_SPEED;
...
void RVisualMesh::Destroy()
{
	...
	if(m_pTracks[0]) {
		RWeaponTracksPool::Release(m_pTracks[0]);
		m_pTracks[0] = NULL;
	}
	if(m_pTracks[1]) {
		RWeaponTracksPool::Release(m_pTracks[1]);
		m_pTracks[1] = NULL;
	}
...
void RVisualMesh::Render(bool low,bool render_buffer) {
	...
	if(m_pMesh) {
		...
		if (m_fVis < RenderingConstants::MIN_VISIBILITY_THRESHOLD) {
			m_bIsRender = false;
			return;
		}
		...
		if(!isRenderedLowPolyModel) {
			...
			if(m_pMesh->GetPhysiqueMesh()) {
				...
				m_vBMax = m_pMesh->m_vBBMaxNodeMatrix * RenderingConstants::BBOX_SCALE_PHYSIQUE;
				m_vBMin = m_pMesh->m_vBBMinNodeMatrix * RenderingConstants::BBOX_SCALE_PHYSIQUE;
			}
		}
...
bool RVisualMesh::UpdateSpWeaponFire()
{
		...
		if(pAniUp->m_pAniSet) {
			...
			if( strcmp(pAniUp->m_pAniSet->GetName(),"attackS")==0 ) {
				...
				if( m_bGrenadeFire ) {
					...
					if( pAniUp->m_nFrame > AnimationConstants::GRENADE_THROW_FRAME ) {
	...
	if(m_bGrenadeFire) {
		...
		if( m_GrenadeFireTime + AnimationConstants::GRENADE_FIRE_DELAY_MS < GetGlobalTimeMS() ) {
...
{
	// TODO: Enchant system currently disabled, remove early return when implemented
	return;
...
void RVisualMesh::GetEnChantColor(u32* color)
{
	...
	if(m_EnchantType==REnchantType_Fire) {
		color[0] = RenderingConstants::ENCHANT_FIRE_COLOR_1;
		color[1] = RenderingConstants::ENCHANT_FIRE_COLOR_2;
	}
	else if(m_EnchantType==REnchantType_Cold) {
		color[0] = RenderingConstants::ENCHANT_COLD_COLOR_1;
		color[1] = RenderingConstants::ENCHANT_COLD_COLOR_2;
	}
	else if(m_EnchantType==REnchantType_Lightning) {
		color[0] = RenderingConstants::ENCHANT_LIGHTNING_COLOR_1;
		color[1] = RenderingConstants::ENCHANT_LIGHTNING_COLOR_2;
	}
	else if(m_EnchantType==REnchantType_Poison) {
		color[0] = RenderingConstants::ENCHANT_POISON_COLOR_1;
		color[1] = RenderingConstants::ENCHANT_POISON_COLOR_2;
	}
	else {
		color[0] = RenderingConstants::ENCHANT_DEFAULT_COLOR_1;
		color[1] = RenderingConstants::ENCHANT_DEFAULT_COLOR_2;
	}
}
...
void RVisualMesh::DrawTracks(bool draw,RVisualMesh* pVWMesh,int mode,rmatrix& m)
{
	...
	if( draw && m_bDrawTracksMotion[mode] ) {
		...
		if(!m_pTracks[mode]) {
			m_pTracks[mode] = RWeaponTracksPool::Acquire();
			m_pTracks[mode]->Create(RenderingConstants::WEAPON_TRACK_DEFAULT_SIZE);
		}
	...
	if(m_pMesh) {
		...
		if(m_pMesh->GetPhysiqueMesh()) {
			if(m_bIsRenderFirst) {
				...
				m_vBMax = m_pMesh->m_vBBMaxNodeMatrix * RenderingConstants::BBOX_SCALE_CALC;
				m_vBMin = m_pMesh->m_vBBMinNodeMatrix * RenderingConstants::BBOX_SCALE_CALC;
			}
		}
		else {
			if(m_bIsRenderFirst)
...
void RVisualMesh::BuildNodeNameCache()
{
	if (!m_pMesh) return;

	m_nodeNameToIdCache.clear();
	m_nodeNameToIdCache.reserve(m_pMesh->m_data_num);

	for (int i = 0; i < m_pMesh->m_data_num; ++i) {
		if (m_pMesh->m_data[i] && m_pMesh->m_data[i]->GetName()) {
			m_nodeNameToIdCache[m_pMesh->m_data[i]->GetName()] = i;
		}
	}
}

int RVisualMesh::FindMeshNodeId(const char* nodeName)
{
	if (!nodeName) return -1;

	// Use cache if available
	auto it = m_nodeNameToIdCache.find(nodeName);
	if (it != m_nodeNameToIdCache.end()) {
		return it->second;
	}

	// Fallback to original method if cache is empty or not found
	return m_pMesh ? m_pMesh->_FindMeshId(nodeName) : -1;
}
...
void RVisualMesh::UpdateMotionTable()
{
	...
	// Build cache if empty or invalidated
	if (m_nodeNameToIdCache.empty()) {
		BuildNodeNameCache();
	}
	...
	if(pAL) {
		...
		for(int i=0;i<node_cnt;i++) {
			...
			pid = FindMeshNodeId(pANode->GetName()); // Use optimized lookup
