...
void RWeaponTracks::Update()
{
	...
	for(int i=0;i < m_current_vertex_size;i++) {
		...
		if(i) {
			...
			ct = at * 1.2f; // Color multiplier for track fading
		}
		else {
			at = 0.1f;
			ct = at * 1.2f; // Color multiplier for track fading
...
240,240,240,240,240,240,240,244,244,244,244,244,244,244,244,244,244,248,248,248,
...
// ==================== RWeaponTracksPool Implementation ====================

std::vector<std::unique_ptr<RWeaponTracks>> RWeaponTracksPool::s_availablePool;
std::vector<RWeaponTracks*> RWeaponTracksPool::s_usedPool;
bool RWeaponTracksPool::s_initialized = false;

void RWeaponTracksPool::Initialize() {
	if (s_initialized) return;

	s_availablePool.reserve(INITIAL_POOL_SIZE);
	s_usedPool.reserve(INITIAL_POOL_SIZE);

	// Pre-allocate initial pool
	for (size_t i = 0; i < INITIAL_POOL_SIZE; ++i) {
		s_availablePool.emplace_back(std::make_unique<RWeaponTracks>());
	}

	s_initialized = true;
}

RWeaponTracks* RWeaponTracksPool::Acquire() {
	Initialize();

	if (s_availablePool.empty()) {
		// Pool exhausted, create new object
		auto newTrack = std::make_unique<RWeaponTracks>();
		RWeaponTracks* ptr = newTrack.get();
		s_availablePool.push_back(std::move(newTrack));
	}

	// Move from available to used
	auto track = std::move(s_availablePool.back());
	RWeaponTracks* ptr = track.release();
	s_availablePool.pop_back();
	s_usedPool.push_back(ptr);

	return ptr;
}

void RWeaponTracksPool::Release(RWeaponTracks* obj) {
	if (!obj) return;

	// Find and remove from used pool
	auto it = std::find(s_usedPool.begin(), s_usedPool.end(), obj);
	if (it != s_usedPool.end()) {
		s_usedPool.erase(it);
		...
		s_availablePool.emplace_back(obj);
	}
}

void RWeaponTracksPool::Cleanup() {
	s_availablePool.clear();

	// Clean up any remaining used objects
	for (auto* obj : s_usedPool) {
		delete obj;
	}
	s_usedPool.clear();
